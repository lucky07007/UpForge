// app/startup/page.tsx — REDESIGN v8
// ─────────────────────────────────────────────────────────────────────────────
// Design: "Newspaper Editorial Premium" — exact match to homepage aesthetic
//   • #F3EFE5 parchment bg, #1A1208 ink, #C8C2B4 rules — identical to homepage
//   • Playfair Display + Georgia serif — same as homepage
//   • Card hover: translate(-2,-2) + 4px 4px 0 #1A1208 — same as homepage .nbtn
//   • Museum-quality layout: each startup is an editorial exhibit
//   • PAGE_SIZE = 12 (fewer, more breathing room per card)
//   • All filter / search / sort / pagination logic identical to v6
//   • Gold accent strip (#E8C547), section dividers, eyebrow labels
//   • Featured cards: full editorial hero treatment
//   • Regular list: large horizontal cards, NOT tiles — generous space
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* Section header — EXACT homepage .sh pattern */
        ...
        .stats-bar { display:flex; background:#1A1208; max-width:640px; margin:0 auto }
        .stat-cell { flex:1; padding:16px 10px; text-align:center; border-right:1px solid rgba(255,255,255,.08) }
        .stat-cell:last-child { border-right:none }
        .stat-v { font-family:'Playfair Display',serif; font-size:clamp(1.3rem,2.8vw,2rem); font-weight:900; color:#fff; line-height:1; margin-bottom:4px }
        .stat-l { font-family:system-ui,sans-serif; font-size:7px; font-weight:700; text-transform:uppercase; letter-spacing:.22em; color:rgba(255,255,255,.32) }
        ...
        /* ── TOOLBAR — compact single row, sticky ── */
        ...
        /* Single combined row: search + filters all in one line */
        .toolbar-row {
          display:flex; align-items:center; height:46px;
          border-bottom:none; gap:0;
        }
        /* Magnifier icon */
        .t-icon { display:flex; align-items:center; padding:0 12px; color:#AAA; font-size:15px; flex-shrink:0; border-right:1px solid #D8D2C4; height:100% }
        /* Search input — slim */
        .t-inp { flex:1; height:100%; border:none; background:transparent; font-family:Georgia,'Times New Roman',serif; font-size:14px; font-style:italic; color:#1A1208; outline:none; padding:0 12px; min-width:0 }
        .t-inp::placeholder { color:#C8C2B4 }
        /* Divider */
        .t-div { width:1px; height:24px; background:#D8D2C4; flex-shrink:0 }
        /* Compact filter selects */
        .t-sel { height:100%; border:none; background:transparent; font-family:system-ui,sans-serif; font-size:10.5px; color:#6B5C40; padding:0 8px 0 10px; outline:none; cursor:pointer; flex-shrink:0 }
        /* Sort pills — compact */
        .t-sort { height:100%; padding:0 12px; display:flex; align-items:center; font-family:system-ui,sans-serif; font-size:8px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:#AAA; text-decoration:none; transition:all .15s; flex-shrink:0; white-space:nowrap }
        .t-sort:hover { color:#1A1208 }
        .t-sort.on { color:#1A1208; font-weight:900 }
        /* Search button — compact */
        .t-btn { height:100%; padding:0 20px; background:#1A1208; color:#fff; border:none; font-family:system-ui,sans-serif; font-size:8px; font-weight:900; letter-spacing:.2em; text-transform:uppercase; cursor:pointer; flex-shrink:0; transition:opacity .15s; margin-left:auto; white-space:nowrap }
        ...
        .t-filter-lbl { display:flex; align-items:center; gap:5px; padding:0 10px 0 14px; font-family:system-ui,sans-serif; font-size:8px; font-weight:700; text-transform:uppercase; letter-spacing:.2em; color:#C8C2B4; flex-shrink:0; border-right:1px solid #D8D2C4; height:100% }
        ...
        .feat-chips { font-family:system-ui,sans-serif; font-size:8.5px; color:#AAA; display:flex; gap:6px }
        ...
        /* ── CTA block — exact homepage footer CTA ── */
      ...
      <header className="mast" role="banner">
        <div className="mast-nameplate ri-0">
          ...
          {/* Eyebrow rule — identical to homepage */}
          ...
          {/* Decorative rule — exact homepage pattern */}
      ...
      {/* ══════════════════════════════
          SEARCH TOOLBAR — compact single row, sticky
      ══════════════════════════════ */}
      <div className="toolbar" id="rg-toolbar">
        <div className="toolbar-inner">
          <form action="/startup" method="GET" className="toolbar-row">
            {/* Filter icon + label */}
            <span className="t-filter-lbl">
              <svg width="11" height="10" viewBox="0 0 11 10" fill="none" aria-hidden="true">
                <path d="M1 1h9M2.5 4.5h6M4 8h3" stroke="#AAA" strokeWidth="1.4" strokeLinecap="round"/>
              </svg>
              Filter
            </span>
            ...
            {isFiltered && <Link href="/startup" className="t-clear">✕</Link>}

            {/* Divider + search — right side */}
            <span className="t-div" style={{ marginLeft:"auto" }} />
      ...
      <div className="main-wrap">
        <div className="main-grid">
          ...
          <div>
            ...
            {featured.length > 0 && (
              <section aria-label="Featured startups" style={{ marginBottom:"clamp(20px,3.5vw,36px)" }}>
                ...
                {/* Gold strip above featured grid — editorial touch */}
          ...
          {/* ── RIGHT: ASIDE — exact homepage sidebar style ── */}
          <aside className="rg-aside ri-3" style={{ position:"sticky", top:88 }}>
            ...
            <div className="aside-box dk" style={{ position:"relative", overflow:"hidden" }}>
              ...
              <p className="aside-p">Get independently verified. Free forever.</p>
            ...
            {/* By the Numbers — exact homepage sidebar pattern */}
            <div className="aside-box dk" style={{ position:"relative", overflow:"hidden" }}>
        ...
        {/* ── CTA BLOCK — exact homepage pattern ── */}
