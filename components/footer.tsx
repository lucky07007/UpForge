        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12 border-t border-white/10 pt-20 pb-20">
            {Object.entries(footerLinks).map(([section, links]) => (
              <div key={section} className="space-y-8">
                ...
                <ul className="space-y-5">
                  {links.map((link) => (
                    <li key={link.name}>
                      <Link href={link.href} className="text-sm text-gray-400 hover:text-white transition-colors font-medium inline-flex items-center gap-1 group">
                        {link.name}
            ...
            <div className="space-y-8">
              ...
              <ul className="space-y-5">
                <li>
                  <Link href="/contact" className="text-sm text-gray-400 hover:text-white transition-colors font-medium inline-flex items-center gap-1 group">
                    Contact
                    <ArrowRight className="h-3 w-3 opacity-0 -ml-1 group-hover:opacity-100 group-hover:ml-1 transition-all" />
                  </Link>
                </li>
                <li>
                  <Link href="/faq" className="text-sm text-gray-400 hover:text-white transition-colors font-medium inline-flex items-center gap-1 group">
                    FAQ
                    <ArrowRight className="h-3 w-3 opacity-0 -ml-1 group-hover:opacity-100 group-hover:ml-1 transition-all" />
                  </Link>
                </li>
                <li className="flex gap-4 pt-2">
                  <a href="https://www.linkedin.com/company/upforge-india/" target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#c6a43f] hover:border-[#c6a43f] transition-all group">
                    <Linkedin className="h-4 w-4 text-white group-hover:text-black" />
                  </a>
                  <a href="#" className="h-8 w-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#c6a43f] hover:border-[#c6a43f] transition-all group">
